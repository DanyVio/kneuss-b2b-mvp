...
describe("Product detail page", function () {
  ...
  it("product detail page as a registered user", function () {
    ...
    cy.get(":nth-child(2) > .item > .product").should("be.visible").click();
    cy.get(".outline-offset-2").should("be.visible").click();
    cy.get(".text-gray-500").should("be.visible").click();
    cy.get("#add-to-wishlist").should("be.visible").click();
    cy.get(".secondary > .action > span").should("be.visible").click();
    cy.get(".actions-primary > .action").should("be.visible").click();
  ...
  it("as a not registered user", function () {
    ...
    cy.get(".outline-offset-2").should("be.visible").click();
    cy.get(".text-gray-500").should("be.visible").click();
    cy.get(".product-data > .action").should("be.visible").click();
